package com.infoCofrade.administration.secretaria.grupo.dao.impl;
...
@Component("grupoDaoImpl")
public class GrupoDaoImpl implements GrupoDao{
	private static GrupoDao instance = new GrupoDaoImpl();
	...
	public synchronized GrupoVO findByPrimaryKey(Transaction transaction, Long id){
		...
		try {
			...
			StringBuilder queryString = new StringBuilder("from GrupoVO");
			if(id != null) {
				...
				grupo = (GrupoVO)session.createQuery(queryString.toString()).uniqueResult();
			}
	...
	public synchronized List<GrupoVO> findAll(Transaction transaction, String order){
	...
	public List<GrupoVO> findUsingTemplate(Transaction transaction, GrupoVO grupo, String order){
		...
		try {
			...
			if(grupo == null){
				throw new Exception("DAO Exception: Grupo is null");
			}
			...
			StringBuilder queryString = new StringBuilder("SELECT grupo.* FROM "+schema+"GRUPO grupo");
			StringBuilder conditions = new StringBuilder();
			if (grupo.getId() != null) {
				conditions.append(" grupo.id = " + grupo.getId() + " AND");
			}
			if(grupo.getNombre() != null && !grupo.getNombre().isEmpty()){
				conditions.append(" UPPER(grupo.nombre) like '%"+grupo.getNombre().toUpperCase()+"%' AND");
			}
			...
			SQLQuery query = session.createSQLQuery(queryString.toString());
			query.addEntity("grupo", GrupoVO.class);
	...
	public List<GrupoVO> findUsingExactTemplate(Transaction transaction, GrupoVO grupo, String order){
		...
		try {
			...
			if(grupo.getNombre() != null && !grupo.getNombre().isEmpty()){
				conditions.append(" UPPER(grupo.nombre) = '"+grupo.getNombre().toUpperCase()+"' AND");
			}
	 ...
	 */
	public synchronized void createElement(Transaction transaction, GrupoVO grupo) throws DaoException{
		session = ConfHibernate.getSessionFactory().getCurrentSession();
		Transaction currentTransaction = transaction;
	 ...
	 */
	public synchronized void deleteElement(Transaction transaction, GrupoVO grupo) {
		session = ConfHibernate.getSessionFactory().getCurrentSession();
		...
		try {
			...
			StringBuilder queryString = new StringBuilder(
					"DELETE FROM "+schema+"GRUPO ");
			StringBuilder conditions = new StringBuilder();
			if (grupo.getId() != null) {
				conditions.append(" id = " + grupo.getId() + " AND");
			}
			if(grupo.getNombre() != null && !grupo.getNombre().isEmpty()){
				conditions.append(" nombre = '"+grupo.getNombre()+"' AND");
			}
